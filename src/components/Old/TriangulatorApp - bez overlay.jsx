  ...
  for (let col = 0; col < CELL_COUNT; col++) {
    ['A', 'B'].forEach(triType => {
      ...
      if (found) {
        ...
        img = getImgById(pieceId); // może być null
      }

      if (!img) {
        const allIds = Object.keys(piecesRef.current || {});
        if (!allIds.length) return;

        const randId = allIds[Math.floor(Math.random() * allIds.length)];
        img = piecesRef.current[randId];
        if (!img) return;
      }
      const w = img.width;
      const h = img.height;
