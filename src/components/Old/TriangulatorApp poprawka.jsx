...
async function onExportClick() {
  ...
  const maskPoly = maskPolyRef.current || {};
  const piecesById = await cutImageIntoTrianglesFromCanvas(canvas, maskPoly);

  const ids = Object.keys(piecesById);
  if (ids.length === 0) {
    alert('No triangles produced');
    return;
  }

  // 🔑 ZAPIS DO REF – JAK W PYTHONIE
  piecesRef.current = piecesById;

  console.log('Produced triangle IDs:', ids);

  // przejście do następnej maski
...
// ================= Triangle numbering (Python-compatible) =================
// Każdy trójkąt dostaje ID: maskId*100 + kolejnyNumer (101,102... 201,202...)

// piecesRef.current = { [id:number]: HTMLCanvasElement }

// ================= Export & Sheets =================

function generateSheets() {
    // helper: pobierz trójkąt po ID (np. 101, 202 itd)
    const getImgById = (id) => {
    ...
    const createSheet = (mapping, overlays, filename) => {
        ...
        for (let row = 0; row < CELL_COUNT; row++) {
            for (let col = 0; col < CELL_COUNT; col++) {
                ['A', 'B'].forEach(triType => {
                    ...
                    // mapping[3] musi być NUMEREM trójkąta jak w Pythonie (np. 101, 102, 201...)
                    if (!found) return; // skip if not in mapping

                    const pieceId = found[3];
                    const imgOrCanvas = getImgById(pieceId);
                    if (!imgOrCanvas) return;

                    const rot = getRotationCW(row, col, triType === 'A' ? 0 : 1);(row, col, triType === 'A' ? 0 : 1);
        ...
        // Overlays (optional, best-effort)
        overlays.forEach(([letter, px, py]) => {
            const overlayImg = new Image();
            overlayImg.src = `${letter}.png`;
            overlayImg.onload = () => {
